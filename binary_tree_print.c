...
static int print_t(const binary_tree_t *tree, int offset, int depth, char **buf)
{
	...
	if (!tree)
		return (0);
	is_left = (tree->pt && tree->pt->left == tree);
...
/**
 * b_height - Measures the height of a binary tree
 *
 * @tree: Pointer to the node to measures the height
 *
 * Return: The height of the tree starting at @node
 */
static size_t b_height(const binary_tree_t *tree)
{
	size_t height_l;
	size_t height_r;

	height_l = tree->left ? 1 + b_height(tree->left) : 0;
	height_r = tree->right ? 1 + b_height(tree->right) : 0;
